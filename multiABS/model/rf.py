...
class RF:
    def __init__(self, config, embed_features):
        ...
        self.config = config
        self.clf = GridSearchCV(estimator=self.est,
    ...
    def fit_eval(self, x, y, x_test, y_test, metrics, oh_encoded=True):
        ...
        for feature in self.embed_features:
            ...
            if name == 'hist' or name == 'item_id':
                continue
            if len(x[name].shape) == 1:  # one dimension
        ...
        self.best_est = self.clf.best_estimator_

        best_param_str = "Best Params: "
        for param, val in self.best_params.items():
            best_param_str += param + ": {0: .4f}".format(val) + ' '
        
        self.config.logger.info('onehot_enabled?: ' + str(oh_encoded))
        self.config.logger.info(best_param_str)
        self.config.logger.info("Best Score: " + str(self.clf.best_score_))
    ...
    def predict(self, x_test, x, y, metrics):
        ...
        y_preds = self.best_est.predict_proba(x)
        y_label = self.best_est.predict(x)
